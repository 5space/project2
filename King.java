		...
		if (Math.abs(move.toRow - move.fromRow) <= 1 &&
			...
			if (player() == Player.WHITE) {
				...
				if (move.fromRow == 7 && move.fromColumn == 4 && !hasMoved) {
					// Checking if left castling is clear and rook is still in
					// the starting position and has not moved
					if (move.toRow == 7 && move.toColumn == 2 &&
							board[move.toRow][move.toColumn] == null &&
							board[move.toRow][move.toColumn + 1] == null &&
							board[7][move.toColumn - 1] == null &&
							board[7][0] != null &&
							board[7][0].type().equals("Rook") &&
							board[7][0].player() == Player.WHITE &&
							!((Rook) board[7][0]).hasMoved()) {
						return true;
					}
					// Checking for the right castling
					return move.toRow == 7 && move.toColumn == 6 &&
							board[move.toRow][move.toColumn] == null &&
							board[move.toRow][move.toColumn - 1] == null &&
							board[7][7] != null &&
							board[7][7].type().equals("Rook") &&
							board[7][7].player() == Player.WHITE &&
			...
			// Checking for castling of black player
				...
				if (move.fromRow == 0 && move.fromColumn == 4 && !hasMoved) {
					...
					if (move.toRow == 0 && move.toColumn == 2 &&
							board[move.toRow][move.toColumn] == null &&
							board[move.toRow][move.toColumn + 1] == null &&
							board[0][move.toColumn - 1] == null &&
							board[0][0] != null &&
							board[0][0].type().equals("Rook") &&
							board[0][0].player() == Player.BLACK &&
							!((Rook) board[0][0]).hasMoved()) {
						return true;
					}
					// Checking for the right castling
					return move.toRow == 0 && move.toColumn == 6 &&
							board[move.toRow][move.toColumn] == null &&
							board[move.toRow][move.toColumn - 1] == null &&
							board[0][7] != null &&
							board[0][7].type().equals("Rook") &&
							board[0][7].player() == Player.BLACK &&
